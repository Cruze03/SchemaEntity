...
inline CEntityInstance* UTIL_GetEntityByIndex(int index)
{
    ...
    {
        if (pEntity->m_EHandle.GetEntryIndex() == index) return pEntity->m_pInstance;
    };
...
inline CEntityInstance* UTIL_FindEntityByClassname(const char* name)
{
    ...
    for (; pEntity; pEntity = pEntity->m_pNext)
    {
        if (!strcmp(pEntity->m_designerName.String(), name)) return pEntity->m_pInstance;
    };
...
inline CEntityInstance* UTIL_FindEntityByEHandle(CEntityInstance* pFind)
{
    ...
    for (; pEntity; pEntity = pEntity->m_pNext)
    {
        if (pEntity->GetRefEHandle() == pFind) return pEntity->m_pInstance;
    };
...
inline std::vector<CEntityInstance*> UTIL_FindEntityByClassnameAll(const char* name)
{
    ...
    for (; pEntity; pEntity = pEntity->m_pNext)
    {
        if (!strcmp(pEntity->m_designerName.String(), name)) entities.push_back(pEntity->m_pInstance);
    };
    return entities;
}
...
class CBaseEntity : public CEntityInstance
{
  public:
    ...
    void Teleport(const Vector* position, const QAngle* angles, const Vector* velocity) { CALL_VIRTUAL(void, 167, this, position, angles, velocity); }
    ...
    void TakeDamage(int iDamage) { m_iHealth(m_iHealth() - iDamage); }

    uint8 GetCollisionGroup()
    {
        if (!m_pCollision()) return 0;
